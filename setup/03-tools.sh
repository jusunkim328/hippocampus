...
echo "=== Hippocampus Tool Registration ==="
echo "Kibana: ${KIBANA_URL}"
echo ""

ERRORS=0

register_tool "${TOOLS_DIR}/recall.json"             || ((ERRORS++))
...
# remember tool is registered in 04-mcp-tools.sh alongside the MCP connector
# (Elastic Workflows execution engine has a Technical Preview bug, so we migrated to MCP)
